module aux::onchain_signer {
    ...
    public(friend) fun create_onchain_signer(sender: &signer) acquires OnchainSigner {
        ...
        get_signer(signer::address_of(sender));
    }
    ...
    public(friend) fun get_signer(user_address: address): signer acquires OnchainSigner {
        let s = borrow_global<OnchainSigner>(user_address);
        let cap = &s.signer_capability;
